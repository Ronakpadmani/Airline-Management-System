...
public class FlightsController {
	...
	public static void  AddNewFlight(Database database , Scanner s) throws SQLException {
		...
		String insert ="INSERT INTO `flight`(`id`, `airplane`, `origin`, `destination`, `departuretime`, `arrivaltime`, `isDelayed`, `bookedEconomy`, `bookedBusiness`, `stuff`, `passenger`) VALUES ('"+flight.getId()+"','"+planeID+"','"+originID+"','"+destinationID+"','"+dTime+"','"+aTime+"','false','0','0','<%%/>','<%%/>');";
	...
	public static ArrayList<Flight> getAllFlight(Database database) throws SQLException{
		...
		while(rs.next()) {
			...
			pass.add(rs.getString("passenger"));
	...
	public static void showAllFlights(Database database) throws SQLException {
		ArrayList<Flight> flight = getAllFlight(database);
	...
	public static void bookFlight(Database database , Scanner s) throws SQLException
	{
		...
		String update = "UPDATE `flight` SET `bookedEconomy`='"+flight.getBookedEconomy()+"',`bookedBusiness`='"+flight.getBookedBusiness()+"',`passenger`='"+sb.toString()+"' WHERE `id` = "+flight.getId()+";";
		database.getStatement().execute(update);
	...
	public static Flight getFlight(Database database, int id) throws SQLException {
		Flight flight = new Flight();
		String select = "SELECT `id`, `airplane`, `origin`, `destination`, `departuretime`, `arrivaltime`, `isDelayed`, `bookedEconomy`, `bookedBusiness`, `stuff`, `passenger` FROM `flight` WHERE `id` = "+id+";";
		ResultSet rs=database.getStatement().executeQuery(select);
		rs.next();
		int ID = rs.getInt("id");
